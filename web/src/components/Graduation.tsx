  ...
  return (
    <section className="grad">
      <span className="label">Through the retention window<Info label="Through the retention window">
        <p>Serve rate at each probe point inside the retention window{isDefault ? ", at 12%, 45%, 72% and 92% of the window" : ""}.</p>
        ...
        {points.map((p, i) => {
          ...
          return (
            <div key={p.key} className="grad-cell">
              <span className="k">{p.key}{isDefault && <span className="at"> · {DEFAULT_AT[i]}</span>}</span>

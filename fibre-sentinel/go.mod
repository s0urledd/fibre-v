// Fibre Sentinel — an independent observer for Celestia Fibre.
//
//   - sentinel-scan discovers publications on chain (single-message
//     MsgPayForFibre txs), decodes the full PaymentPromise, tracks the fibre
//     params from EventUpdateFibreParams, and records must_serve_until plus the
//     fibre-assign shard assignment for each one.
//   - sentinel-probe turns each record into a probe schedule over that blob's
//     retention window and appends one raw Measurement per probe, classified by
//     a fault taxonomy.
//
// celestia-app is a pinned proxy dependency (pseudo-version for
// PinnedCelestiaAppCommit); its replace block is copied verbatim below because
// Go does not apply a dependency's replaces. fibre-assign and fibre-tlsverify
// are sibling modules in this repo, resolved by the relative replace directives.
module github.com/plsgiveup/fibre/fibre-sentinel
...
// Sibling modules in this repo. A full-repo checkout (git clone, or the CI
// checkout) resolves these; there is no cross-repo version to tag.
replace github.com/plsgiveup/fibre/fibre-assign => ../fibre-assign

replace github.com/plsgiveup/fibre/fibre-tlsverify => ../fibre-tlsverify
